...
\subsection{Tests}
To evaluate the results of this thesis, tests will be made to compare how the system behaves with the old hardware and controller compared to the new system setup. The first type of test will consist of the \gls{rcv} standing still and then giving different torque requests of 2, 5, 10, and 15 [Five of each?] and then measuring the response time by measuring in the resulting plots. 
The other test will be braking performance in terms of reduced braking distance. Tests will be done at three different speeds, which are 5, 15 and 30 km/h, and five tests will be done for each test case. [For these tests i need speed(both starting velocity and plot), pressure in system, ]
